...
const getModerators = async (req, res) => {
  try {
    // Return all users who can be made moderators (excludes admin)
    const moderators = await User.find({ role: { $ne: "admin" } }).select(
      "_id name email"
    );
...
const addModerator = async (req, res) => {
  try {
    ...
    await User.findByIdAndUpdate(moderatorId, { role: "moderator" });
...
const addCommunity = async (req, res) => {
  try {
    ...
    const newCommunity = new Community({
      name,
      description,
      banner: banner || "/community-default-banner.svg",
      moderators: [],
      members: [],
    });
